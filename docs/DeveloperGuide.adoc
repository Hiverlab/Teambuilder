...
Hiverlab Pte Ltd. With the Gallup StrengthsFinder test in mind, Team Builder aims to delegate desirable team members in
an efficient manner by leveraging on one another’s strengths.

This developer guide specifies the design and implementation of Team Builder. The intended audience of this document is
...
This web app was made using HTML/CSS/TS and is largely implemented based on the Angular framework. This framework was
chosen to make the code cleaner, as compared to not using any Javascript frameworks at all.
...
We have implemented *Alternative 1* as we want the search function to be more dynamic and more generic to
...
image::SetSequenceDiagram.png[width="800"]
...
. `[fuchsia]#TotalBudget#updateBudget()#` first checks whether the new budget set is more than or equal to the sum of the category budgets and throws an
 exception if it is not. It then gets the budget data (`[fuchsia]#totalBudget#`, `[fuchsia]#currentBudget#`) from `[fuchsia]#Budget p#` and sets it using
...
the records that was stored and passed in by the `[fuchsia]#FinanceTracker#`. This is shown in the code snippet below:
...
image::BudgetUiUpdateActivityDiagram.png[wdith="800"]
...
image::summaryPanel.png[width="450"]
